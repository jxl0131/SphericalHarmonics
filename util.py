...
def tonemapping(im , alpha = None):
    
    power_im = np.power(im, 1 / 1.4)
    # print (np.amax(power_im))
    
    if alpha == None:#假如没给alpha，自行计算alpha
        non_zero = power_im > 0
        if non_zero.any():
            r_percentile = np.percentile(power_im[non_zero], 99.9) #找到99.9位置处的那个数
        else:
...
if __name__ == '__main__':
    pass
    import light_detect.detect_util #之前的项目文件夹名称可能是light_detect，文件夹下的util当时叫detect_util。只有当前文件不被直接运行，此处解析错误也是无所谓的。
